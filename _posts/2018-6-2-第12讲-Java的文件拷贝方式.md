...
```
	...
	- 实现步骤：
		...
		2. 如果相同的文件系统服务类，则根据不同情况（操作系统、jar文件、压缩文件）提供抽象类`FileSystemProvider`的具体实现类
		3. 如果不同的文件系统服务类，则使用`java.nio.file`包下的一个package-private类CopyMoveHelper来实现文件拷贝
		```java
...
- 所以归根到底，当讨论文件拷贝方式的时候，其实只有两种实现途径
    ...
    - 使用java.nio的方式，将文件化作FileInputStream和FileOutputStream，然后分别从FileInputStream/FileOutputStream中获取FileChannel，并从源channelTransferTo到目标channel中
